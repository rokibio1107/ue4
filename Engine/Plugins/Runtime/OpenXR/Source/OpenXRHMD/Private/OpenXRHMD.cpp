...
void FOpenXRHMD::PostRenderView_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneView& InView)
{
	DrawEmulatedQuadLayers_RenderThread(RHICmdList, InView);
}
		...
		for (const FOpenXRLayer& Layer : NativeQuadLayers)
		{
			...
			if (Layer.Swapchain.IsValid() && Layer.Desc.Texture.IsValid())
			{
				if (Layer.bUpdateTexture && bIsRunning)
				{
					...
					const FIntRect DstRect(FIntPoint(0, 0), Layer.SwapchainSize.IntPoint());
...
void FOpenXRHMD::DrawEmulatedQuadLayers_RenderThread(FRHICommandListImmediate& RHICmdList, const FSceneView& InView)
{
	check(IsInRenderingThread());
	...
	GetRelativeEyePose(IXRTrackingSystem::HMDDeviceId, InView.StereoPass, EyeOrientation, EyeShift);
...
void FOpenXRHMD::OnBeginRendering_RenderThread(FRHICommandListImmediate& RHICmdList, FSceneViewFamily& ViewFamily)
{
	...
	if (bIsRunning)
	{
		...
		{
			NativeQuadLayer.bUpdateTexture = NativeQuadLayer.Desc.Flags & IStereoLayers::LAYER_FLAG_TEX_CONTINUOUS_UPDATE;
		}
...
void FOpenXRHMD::OnBeginRendering_GameThread()
{
	...
	[this, bBackgroundLayerVisible = IsBackgroundLayerVisible()](FRHICommandListImmediate&)
	{
...
void FOpenXRHMD::UpdateLayer(FOpenXRLayer& ManagerLayer, uint32 LayerId, bool bIsValid)
{
	for (FOpenXRLayer& NativeLayer : NativeQuadLayers)
	{
		if (NativeLayer.GetLayerId() == LayerId)
		{
			...
			NativeLayer.bUpdateTexture = ManagerLayer.bUpdateTexture;
			if (bStaticSwapchain && NativeLayer.bUpdateTexture)
			{
				NativeLayer.Swapchain.Reset();
				NativeLayer.LeftSwapchain.Reset();
			}
		}
	}
}
