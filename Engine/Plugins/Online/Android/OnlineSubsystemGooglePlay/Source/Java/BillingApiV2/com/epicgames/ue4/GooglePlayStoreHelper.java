			...
			mainHandler.post(new Runnable()
			{
				...
				public void run()
				{
					...
					for (Purchase ownedProduct : f_ownedProducts)
					{
						...
						{
							if( ownedProduct.getSkus().contains(RestoreProductIDs[Idy]) )
							{
								if(Idy < bRestoreConsumableFlags.length)
								{
									bTryToConsume = bRestoreConsumableFlags[Idy];
									Log.debug("[GooglePlayStoreHelper] - GooglePlayStoreHelper::RestorePurchases - Found Consumable Flag for Product " + ownedProduct.getSkus().get(0) + " bConsumable = " + bTryToConsume);
								}
								break;
							}
						}
						...
						if (bTryToConsume)
						{
							Log.debug("[GooglePlayStoreHelper] - GooglePlayStoreHelper::RestorePurchases - Attempting to consume " + ownedProduct.getSkus().get(0));
							...
							mBillingClient.consumeAsync(consumeParams, (new ConsumeResponseListener() {
								...
								public void onConsumeResponse(BillingResult billingResult, String purchaseToken) {
									if(billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK)
									{
										...
										Log.debug("[GooglePlayStoreHelper] - GooglePlayStoreHelper::RestorePurchases - Purchase restored for " + constProduct.getSkus().get(0));
										String receipt = Base64.encode(constProduct.getOriginalJson().getBytes());
										receipts.add(receipt);
										
										f_ownedSkus.add(constProduct.getSkus().get(0));
										f_signatureList.add(constProduct.getSignature());
									}
									else
									{
										Log.debug("[GooglePlayStoreHelper] - GooglePlayStoreHelper::RestorePurchases - consumePurchase failed for " + constProduct.getSkus().get(0) + " with error " + billingResult.getResponseCode());
										receipts.add("");
	...
	public boolean QueryExistingPurchases()
	{
		...
		if (responseCode == BillingClient.BillingResponseCode.OK)
		{
			...
			for (Purchase purchase : ownedProducts)
			{
				...
				ownedSkus.add(purchase.getSkus().get(0));
